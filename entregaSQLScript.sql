...
create table practica.coches (
	...
	num_kms_totales int not null,
	id_modelo int not null,
	fecha_compra date not null,
	constraint uk_matricula_seguro unique (matricula)
);

create table practica.seguros (
	id serial primary key,
	id_coche int not null,
	num_poliza int not null,
	id_aseguradora int not null,
	fecha_alta_seguro date not null,
	constraint uk_poliza unique (num_poliza)
);

create table practica.aseguradoras (
	id serial primary key,
	nombre varchar(50) not null,
	constraint uk_nombre_aseguradoras unique (nombre)
);
...
create table practica.modelos (
	...
	constraint uk_nombre_modelos unique (nombre)
);
...
create table practica.grupos (
	...
	constraint uk_nombre_grupo unique (nombre)
);
...
create table practica.monedas(
	id serial primary key,
	nombre varchar(50) not null,
	constraint uk_nombre_moneda unique (nombre)
);

-- Creo las relaciones entre tablas:
alter table practica.coches add constraint fk_id_modelos foreign key (id_modelo) references practica.modelos(id);
alter table practica.seguros add constraint fk_id_coches foreign key (id_coche) references practica.coches(id);
alter table practica.seguros add constraint fk_id_aseguradoras foreign key (id_aseguradora) references practica.aseguradoras(id);
alter table practica.modelos add constraint fk_id_marcas foreign key (id_marca) references practica.marcas(id);
alter table practica.marcas add constraint fk_id_grupos foreign key (id_grupo) references practica.grupos(id);
alter table practica.revisiones add constraint fk_id_coches foreign key (id_coche) references practica.coches(id);
alter table practica.revisiones add constraint fk_id_monedas foreign key (id_moneda) references practica.monedas(id);


-- Creo la tabla de datos
CREATE TABLE practica.coches_datos (
...
INSERT INTO practica.coches_datos (matricula,grupo,marca,modelo,fecha_compra,color,aseguradora,n_poliza,fecha_alta_seguro,importe_revision,moneda,kms_revision,fecha_revision,kms_totales) VALUES
...
-- Comienzo a poblar las tablas.

-- Empiezo por la tabla de los grupos
insert into practica.grupos(nombre)
select grupo from practica.coches_datos cd group by grupo;

-- Tabla monedas
insert into practica.monedas(nombre)
select moneda from practica.coches_datos cd group by moneda;

-- Tabla aseguradoras
insert into practica.aseguradoras(nombre)
select aseguradora from practica.coches_datos cd group by aseguradora;

-- Tabla marcas
insert into practica.marcas(nombre, id_grupo)
select marca, g.id  from practica.coches_datos cd 
inner join practica.grupos g on g.nombre = cd.grupo 
group by marca, g.id;

-- Tabla modelos
insert into practica.modelos(nombre, id_marca)
select modelo,m.id  from practica.coches_datos cd 
inner join practica.marcas m on cd.marca = m.nombre 
group by modelo, m.id;

-- Tabla coches
insert into practica.coches(matricula,color,num_kms_totales, fecha_compra, id_modelo)
select matricula, color, kms_totales, fecha_compra, m.id id_modelo  from practica.coches_datos cd 
inner join practica.modelos m on cd.modelo = m.nombre 
group by matricula, color, kms_totales , fecha_compra, m.id;

-- Tabla seguros
insert into practica.seguros(num_poliza, id_aseguradora, fecha_alta_seguro, id_coche)
select n_poliza, a.id, fecha_alta_seguro, co.id  from practica.coches_datos cd 
inner join practica.coches co on co.matricula  = cd.matricula
inner join practica.aseguradoras a on a.nombre = cd.aseguradora 
group by n_poliza, a.id, fecha_alta_seguro, co.id;

-- Tabla revisiones
insert into practica.revisiones(kms_revision, fecha, importe, id_coche, id_moneda)
select kms_revision, fecha_revision, importe_revision, co.id, mo.id from practica.coches_datos cd 
inner join practica.coches co on co.matricula = cd.matricula
inner join practica.monedas mo on mo.nombre = cd.moneda
group by kms_revision, fecha_revision, importe_revision, co.id, mo.id;


-- Elimino la tabla de los datos
drop table practica.coches_datos;


-- Select polizas mas actuales
select m.nombre modelo, m2.nombre marca,g.nombre grupo, fecha_compra, matricula, color, num_kms_totales total_kilometros, a.nombre aseguradora, s.num_poliza numero_poliza from practica.coches c 
inner join practica.modelos m on m.id  = c.id_modelo
inner join practica.marcas m2 on m2.id  = m.id_marca 
inner join practica.grupos g on g.id = m2.id_grupo
inner join (
	select num_poliza, id_coche, id_aseguradora from practica.seguros s 
	where fecha_alta_seguro in (
		select maxDate from (
			select id_coche, max(fecha_alta_seguro) maxDate from practica.seguros where s.id_coche = id_coche group by id_coche
		) se
	) group by num_poliza, id_coche, id_aseguradora
) s on s.id_coche = c.id 
inner join practica.aseguradoras a on a.id = s.id_aseguradora 
group by m.nombre , m2.nombre ,g.nombre , fecha_compra, matricula, color, num_kms_totales, a.nombre, s.num_poliza; 
